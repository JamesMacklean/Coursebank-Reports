    ...
    if course_id is None:
        ...
        for p in profiles:
            try:
                user_list.append({
                    ...
                    "username": p.username,
                    "email": p.email,
                    "gender": UserProfile.objects.filter(user_id=p.id).values_list('gender').get(),
                    "birthyear": UserProfile.objects.filter(user_id=p.id).values_list('year_of_birth').get(),
                    "location": UserProfile.objects.filter(user_id=p.id).values_list('country').get(),
                })
            except UserProfile.DoesNotExist:
                user_list.append({
                    "studentid": p.id,
        ...
        for e in enrollments:
            ...
            user_list.append({
                ...
                "username": e.user.username,
                "email": e.user.email,
                "gender": UserProfile.objects.filter(user_id=e.user.id).values_list('gender').get(),
                "birthyear": UserProfile.objects.filter(user_id=e.user.id).values_list('year_of_birth').get(),
                "location": UserProfile.objects.filter(user_id=e.user.id).values_list('country').get(),
                "created": e.created,
            })
        file_name = '/home/ubuntu/tempfiles/export_learner_profiles_{}.csv'.format(tnow)
        with open(file_name, mode='w') as csv_file:
