    ...
    if course_id is None:
        ...
        for p in profiles:
            try:
                user_list.append({
                    ...
                    "username": p.username,
                    "email": p.email,
                    "gender": UserProfile.objects.raw("Select gender from auth_userprofile where user_id = %s LIMIT 1", [p.id]),
                    "birthyear": UserProfile.objects.raw("Select year_of_birth from auth_userprofile where user_id = %s LIMIT 1", [p.id]),
                    "location": UserProfile.objects.raw("Select country from auth_userprofile where user_id = %s LIMIT 1", [p.id]),
                })
            except UserProfile.DoesNotExist:
                user_list.append({
                    "studentid": p.id,
        ...
        for e in enrollments:
            ...
            user_list.append({
                ...
                "username": e.user.username,
                "email": e.user.email,
                "gender": UserProfile.objects.raw("Select gender from auth_userprofile where user_id = %s LIMIT 1", [e.user.id]),
                "birthyear": UserProfile.objects.raw("Select year_of_birth from auth_userprofile where user_id = %s LIMIT 1", [e.user.id]),
                "location": UserProfile.objects.raw("Select country from auth_userprofile where user_id = %s LIMIT 1", [e.user.id   ]),
                "created": e.created,
            })
        file_name = '/home/ubuntu/tempfiles/export_learner_profiles_{}.csv'.format(tnow)
        with open(file_name, mode='w') as csv_file:
